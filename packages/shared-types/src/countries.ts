...
/** ISO-style country list with dial codes (alphabetical by name). */
...
export function countryDialCode(nameOrCode: string): string | undefined {
  const q = nameOrCode.trim().toLowerCase();
  return COUNTRY_OPTIONS.find((c) => c.name.toLowerCase() === q || c.code.toLowerCase() === q)?.dialCode;
}

export function formatPhoneWithCountry(dialCode: string, nationalNumber: string): string {
  const trimmed = nationalNumber.trim();
  const d = dialCode.replace(/^\+?/, '');
  return '+' + d + (trimmed ? ' ' + trimmed : '');
}
